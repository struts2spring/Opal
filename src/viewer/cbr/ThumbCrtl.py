...
class ScrolledThumbnail(wx.ScrolledWindow):
    ...
    def OnMouseDown(self, event):
        ...
        logger.info('selected item:'+str(self._selected))
        logger.info('selected array:'+str(self._selectedarray))
        if update:
            self.ScrollToSelected()
            self.Refresh()
            eventOut = ThumbnailEvent(wxEVT_THUMBNAILS_SEL_CHANGED, self.GetId())
            self.GetEventHandler().ProcessEvent(eventOut)
            if len(self._selectedarray)==1:
                path=os.path.join(self._items[self._selected]._dir,self._items[self._selected]._filename)
                self.GetTopLevelParent().photoPanel.imagePath=path
                self.GetTopLevelParent().photoPanel.changeBitmapWorker()
                logger.info(self.GetTopLevelParent().photoPanel.imagePath)
                pass
